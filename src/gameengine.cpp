...
GameEngine::GameEngine(QObject *parent)
    : QObject(parent), m_currentNode(nullptr), m_playTimerUpdate(new QTimer(this))
{
    connect(m_playTimerUpdate, &QTimer::timeout, this, &GameEngine::updatePlayTime);
}
...
QString GameEngine::storyTitle() const
{
    return m_storyTitle;
}
...
int GameEngine::health() const
{
    return m_health;
}

int GameEngine::strength() const
{
    return m_strength;
}

int GameEngine::intelligence() const
{
    return m_intelligence;
}

int GameEngine::wisdom() const
{
    return m_wisdom;
}

int GameEngine::choicesMade() const
{
    return m_choicesMade;
}

int GameEngine::nodesVisited() const
{
    return m_visitedNodes.size();
}

int GameEngine::playTimeSeconds() const
{
    return m_playTimeSeconds;
}

QStringList GameEngine::inventory() const
{
    return m_inventory;
}

QStringList GameEngine::endingsFound() const
{
    return m_endingsFound;
}

bool GameEngine::isChoicePreviouslySelected(const QString &nodeId, int choiceIndex) const
{
...
void GameEngine::loadStory(const QString &filePath)
{
    ...
    m_storyTitle = StoryLoader::getStoryTitle(filePath, errorMsg);
...
void GameEngine::goBack()
{
    if (!m_history.isEmpty() && !m_stateHistory.isEmpty()) {
        ...
        for (auto it = lastChange.statChanges.constBegin(); it != lastChange.statChanges.constEnd(); ++it) {
...
void GameEngine::applyStatChanges(const QMap<QString, int> &changes)
{
    for (auto it = changes.constBegin(); it != changes.constEnd(); ++it) {
        ...
        if (statName == "health") {
            ...
            if (m_health == 0) {
                emit gameOver("Your Qi Vitality has been depleted! Your cultivation journey ends here...");
            }
        } else if (statName == "strength") {
            m_strength += change;
            emit strengthChanged();
